...
export function Categories({name , id, onClick , isSelected}){
    ...
    return(
        <Link to={`/show/${test}`}>   
 <li id={id} className={`text-sm md:text-lg font-normal cursor-pointer ${isSelected ? 'font-bold text-white' : ''}`}onClick={handleClick}>{name} </li>
 </Link>
    )
}
...
export default function Category(data){
    ...
    return(
        ...
        <ul className="flex flex-wrap gap-10 w-full border-t-2 border-gray-500 text-sm text-textgrey mx-5 py-4 md:mx-10 xl:mx-24">
            {data2.map((item) => (
         <Categories onClick={handleCategoryClick}  key={item.id} name={item.name} id={item.id} isSelected={selectedCategory === item.id} 
         />
  ))}  
        </ul>
