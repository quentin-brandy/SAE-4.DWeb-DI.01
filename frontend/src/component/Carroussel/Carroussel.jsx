...
const Carousel = ({ slides }) => {
  ...
  return (
    <div className="relative z-10 w-screen">
      ...
      <div className="absolute bottom-9  z-40 flex w-full justify-end gap-1 pr-10">
        {slides.map((_, index) => (
          <button
            key={index}
            ...
            style={{
              backgroundImage:
                index === currentSlide
                  ? "linear-gradient(to right, #4299e1 0%, #4299e1 100%)"
                  : "linear-gradient(to right, #cbd5e0 0%, #cbd5e0 100%)",
...
export default function App(data) {
  ...
  const slides = test.map((item, index) => (
    <CarrousselBanner key={index} {...item} />
  ));
